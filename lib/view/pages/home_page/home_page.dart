...
class _HomePageState extends State<HomePage> {
  ...
  Future<void> getAuctions() async {
    if (!loading && page <= numberOfPages) {
      ...
      final responseMap = await get("${ApiPaths.getAuction}$carAuctions&page=$page").onError((error, stackTrace) {
        return {
          'code':0,
        'error':error
        };
                  ...
                  itemBuilder: (context, index) {
                    if (index < _model.auctions.length) {
                      ...
                      return wrapWithModel(
                        ...
                        child: AuctionCardWidget(auctionCardPosition: AuctionCardPosition.home,auction: _model.auctions[index],model: _model),
                      );
                    } else if (loading) {
